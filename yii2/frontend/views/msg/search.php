...
<body id="message" onselectstart="return true;" ondragstart="return false;">
	...
	<div class="container">
		<form action="<?= \yii\helpers\Url::toRoute('msg/search') ?>" method="post" >
		按时间搜索留言:<input type="text" placeholder="请输入开始时间" name="start_time" id="test5">&nbsp&nbsp&nbsp&nbsp<input type="text" placeholder="请输入结束时间" id="test6" name="stop_time"><input type="submit" name="" id="search" value="搜索"> 
		</form>
					...
					<ul>
						...
						<li class="green bounceInDown">
							<h3>
								...
								<?php echo $msg->uname ?>
								<span><?= date('Y-m-d H:m:s',$msg->created_at) ?></span>
								<div class="clr"></div>
							</h3>
							<dl>
  ...
  // 		,range: true
		...
		// 	var stop = $("input[name='stop_time']").val();
		// 	// console.log(start);
		// 	// console.log(stop);
		// 	$.get("<?= \yii\helpers\Url::toRoute('msg/search') ?>",{"start_time":start,"stop_time":stop},function(json){
